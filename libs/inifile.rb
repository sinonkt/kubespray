# This file was copied from https://github.com/TwP/inifile
# all credit to original author
# a little bit of modification so that inifile can load ansible hosts.ini
#encoding: UTF-8
  ...
  class Parser
    ...
    def parse_value( string )
      ...
      # not currently processing a continuation line
        ...
        # *********************** Sinonkt Modification ***************************** 
        when @normal_value
          if $1.include?(' ')
            self.value = $1.split(' ').reduce({}) { |acc, str| key, value = str.split('='); acc.merge!("#{key}"=> value) }
          else
            self.value ?  self.value << $1 : self.value = $1
          end
        when nil
      ...
      content.each_line do |line|
        ...
        if continuation
          ...
          when @property_regexp
            # *********************** Sinonkt Modification *****************************
            # *************** Ensure only key & Ansible Style Key **********************
            if $1.include?(' ')
              front, back = $1.split(' ')
              self.property = front
            else 
            ...
            continuation = parse_value ($1.include?(' ') ? $2.prepend(back, "=") : $2)
          when @key_regexp
            self.property = $1.strip
            error if property.empty?
